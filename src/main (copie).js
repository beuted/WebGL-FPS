...
ASPECT = WIDTH / HEIGHT,
UNITSIZE = 100, // attention ausi dans map ! (?)
WALLHEIGHT = UNITSIZE,
MOVESPEED = 100,
LOOKSPEED = 0.075,
...
function render() {
	...
	for (var j = ai.list.length-1; j >= 0; j--) {
	    var a = ai.list[j];
	    var v = a.mesh.geometry.vertices[0];
	    var c = a.mesh.position;
...
function getAIpath(a) {
    var p = getMapSector(a.mesh.position);
    do { // Cop-out
	do {
	    var x = getRandBetween(0, map.size[1]-1);
	    var z = getRandBetween(0, map.size[2]-1);
	} while (map.blocsMap[0][x][z] > 0 || distance(p.x, p.z, x, z) < 3);
...
function getMapSector(v) {
    var x = Math.floor((v.x + UNITSIZE / 2) / UNITSIZE + map.size[1]/2);
    var z = Math.floor((v.z + UNITSIZE / 2) / UNITSIZE + map.size[1]/2);
    return {x: x, z: z};
}
...
function checkWallCollision(v) {
    ...
    return map.blocsMap[0][c.x][c.z] > 0;
}
...
function drawRadar() {
	...
	for (var j = 0, m = map.blocsMap[0][i].length; j < m; j++) {
	    var d = 0;
	    for (var k = 0, n = ai.list.length; k < n; k++) {
		var e = getMapSector(ai.list[k].mesh.position);
		if (i == e.x && j == e.z) {
		    d++;
		}
	    }
